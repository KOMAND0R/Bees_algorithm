...
class floatbee:
	...
	def gotorandom (self):
		...
		self.position = [random.uniform (self.minval[n], self.maxval[n]) for n in xrange (len (self.position) ) ]
		self.checkposition()
		self.calcfitness()	
...
class statistic:
	...
	def formatfitness (self, runnumber):
		...
		result = ""
		for n in xrange (len (self.fitness[runnumber]) ):
	...
	def formatcolumns (self, runnumber, column):
		...
		for n in xrange ( len (column[runnumber] ) ):
			line = "%6.6d" % n
